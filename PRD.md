...
**최대 10명의 에이전트**가 참여하는 다자간 시트콤 상황극 시뮬레이터.

순차적 턴이 아닌, **"누가 말할지 결정하는(Speaker Selection)"** 로직이 추가된다.

각 턴마다:

현재 상황에 맞춰 **발화자 선정** (Speaker Selector)

선정된 에이전트는 기억 검색 -> 반성 -> 계획 -> 발화 수행

나머지 에이전트는 이를 관찰(Observe)

UI에서 10명의 상태와 대화 흐름을 직관적으로 볼 수 있어야 한다.

1.2 비목표 (Non-goals)

공간 내 물리적 이동/경로 계획 (여전히 텍스트 기반)

실시간성 (Real-time): 턴 기반 시뮬레이션임
...
active_agents: list[str] (현재 참여 중인 에이전트 목록)
...
7. 에이전트 인지 구조 (agent.py + selector.py)
7.0 발화자 선정 (Selector)
입력: 최근 대화 로그, 에이전트 프로필 목록
출력: Next Speaker Name (또는 None if silence)
로직:
라운드 로빈 (단순) 또는
LLM 기반 ("누가 대답할 차례인가?")
...
Step 1: Selector가 화자(Speaker) 결정
Step 2: Speaker는 run_step() 실행 (Retrieve->Act)
단계 A: Query 구성
...
Step 3: 나머지 Listener들은 Store Observation 수행
